...
#ifdef CONFIG_HTTPD
static u8 phy_link_prev[IPQ9574_PHY_MAX] = {0xFF};
static ulong phy_link_last_check = 0;
#define PHY_LINK_CHECK_INTERVAL 1500

int ipq9574_eth_check_link_change(void)
{
	ulong now = get_timer(0);
	struct ipq9574_eth_dev *priv;
	int i, node;
	int aquantia_port[3] = {-1, -1, -1}, aquantia_port_cnt = -1;
	int sfp_port[3] = {-1, -1, -1}, sfp_port_cnt = -1;
	int phy_node = -1;
	u8 cur_link[IPQ9574_PHY_MAX];
	int changed = 0;
	...
	if (!ipq9574_edma_dev[0] || !ipq9574_edma_dev[0]->dev)
		return -1;

	priv = ipq9574_edma_dev[0]->dev->priv;

	node = fdt_path_offset(gd->fdt_blob, "/ess-switch");
	if (node >= 0) {
		aquantia_port_cnt = fdtdec_get_uint(gd->fdt_blob, node, "aquantia_port_cnt", -1);
		if (aquantia_port_cnt >= 1) {
			fdtdec_get_int_array(gd->fdt_blob, node, "aquantia_port",
					  (u32 *)aquantia_port, aquantia_port_cnt);
		}
		sfp_port_cnt = fdtdec_get_uint(gd->fdt_blob, node, "sfp_port_cnt", -1);
		if (sfp_port_cnt >= 1) {
			fdtdec_get_int_array(gd->fdt_blob, node, "sfp_port",
					  (u32 *)sfp_port, sfp_port_cnt);
		}
	}
	phy_node = fdt_path_offset(gd->fdt_blob, "/ess-switch/port_phyinfo");
	...
	for (i = 0; i < IPQ9574_PHY_MAX; i++) {
		if (phy_info[i] && phy_info[i]->phy_type == UNUSED_PHY_TYPE) {
			cur_link[i] = 0;
		} else if (i == sfp_port[0] || i == sfp_port[1] || i == sfp_port[2]) {
